...
public interface ProductRepository extends JpaRepository<Product, Long> {
	...
	@Query("SELECT p FROM Product p " + "WHERE p.category.name = :thirdLavelCategory "
			+ "AND p.category.parentCategory.name = :secondLavelCategory "
			+ "AND p.category.parentCategory.parentCategory.name = :topLavelCategory")
	List<Product> findRelatedProducts(@Param("topLavelCategory") String topLavelCategory,
			@Param("secondLavelCategory") String secondLavelCategory,
			@Param("thirdLavelCategory") String thirdLavelCategory);
